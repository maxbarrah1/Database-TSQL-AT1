...
ELSE IF @PCUSTID = (SELECT CUSTID FROM CUSTOMER)
...
IF @PPRODID = (SELECT PRODID FROM SALE)

    THROW 50320, 'Customer cannot be deleted as sale exists', 1 

ELSE IF @PPRODID = (SELECT PRODID FROM PRODUCT)
